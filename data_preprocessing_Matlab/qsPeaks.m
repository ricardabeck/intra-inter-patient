...
%   % Input test
%   disp('QS peaks detection input: length ECG, Rposition, fs');
%   disp(length(ECG));
%   disp(length(Rposition));
%   disp(fs);
...
for i = 2:length(Rposition)-1
    ...
    % Test Ricarda
    %disp('lastOFF, thisON, thisOFF, nextON');
    %disp(lastOFF); disp(thisON); disp(thisOFF); disp(nextON);
