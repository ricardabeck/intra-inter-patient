...
load(matName);
ecgsig = val;
fid = fopen(infoName, 'rt'); % open file in read mode
...
% extract 4th line, which contains the frequency and interval
[freqint] = sscanf(fgetl(fid), 'Sampling frequency: %f Hz  Sampling interval: %f sec');
Fs = freqint(1);
interval = freqint(2);
fgetl(fid);

% iterate over the ecg signals
for i = 1:size(ecgsig, 1)
  [row(i), signal(i), gain(i), base(i), units(i)]=strread(fgetl(fid),'%d%s%f%f%s','delimiter','\t');
  %[row(i), signal(i), gain(i), base(i), units(i)] = textscan(fgetl(fid),'%d%s%f%f%s','delimiter','\t');
end
